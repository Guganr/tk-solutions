<x-app-layout>
   <x-slot name="header">
      <h2 class="font-extrabold text-xl text-white leading-tight">
         Adicionar Pagamento (Contrato #{{ $contrato->id }})
      </h2>
   </x-slot>
   <div>
      <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="block mb-8">
                <a href="{{ route('pagamentos.show', $contrato) }}" class="bg-yellow-500 hover:bg-white hover:text-yellow-500 text-white font-bold py-0 px-2 rounded">< Voltar</a>
            </div>
            ...
            <form method="post" action="{{ route('pagamentos.store') }}">
               ...
               <div class="shadow overflow-hidden sm:rounded-md">
                  <div class="px-4 py-5 bg-black sm:p-6">
                        <label for="datas_pagamento" class="block font-black text-sm text-white">Datas</label>
                  ...
                  <div class="px-4 py-5 bg-black sm:p-6">
                     <label for="valor" class="block font-black text-sm text-white">Valor do pagamento
                     </label>
                  ...
                  <div class="flex items-center justify-end px-4 py-3 bg-black text-right sm:px-6">
                     <button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Adicionar Pagamentos</button>
                  </div>
               </div>
            </form>
         </div>
